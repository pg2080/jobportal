...
<div class="content">
                                        ...
                                        <div class="container ">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-12 pr-lg-4 ">
                                                    ...
                                                    <?php
                                                    $Cid = $_SESSION['Login']['Company']['CompanyId'];//['CId'];//['CompanyId'];
                                                   //print_r($Cid);
                                                   ...
                                                   //  $sqln = "SELECT iw.*,dept.DepartmentName,j.*,js.*,hr.*,ap.* from tblinterview iw LEFT JOIN tbldepartment dept on iw.DepartmentId = dept.DepartmentId LEFT JOIN tbljob j on iw.JobId = j.JobId LEFT JOIN jobseekerinfor js ON iw.ApplicantId = js.LoginId LEFT JOIN hrinfo hr ON iw.HRId = hr.LoginId LEFT JOIN tblappplication ap ON iw.ApplicationId = ap.ApplicationId WHERE iw.STATUS='PE' and iw.CompanyId=$Cid";
                                                    //$result = mysqli_query($conn, $sqln);
                                                    //      print_r(mysqli_fetch_assoc($result));
                                                     $sqln = "SELECT lo.Email as Email,iw.*,dept.DepartmentName,j.*,js.*,hr.*,ap.* from tblinterview iw LEFT JOIN tbldepartment dept on iw.DepartmentId = dept.DepartmentId LEFT JOIN tbljob j on iw.JobId = j.JobId LEFT JOIN jobseekerinfor js ON iw.ApplicantId = js.LoginId LEFT JOIN tbllogin lo ON iw.ApplicantId = lo.LoginId LEFT JOIN hrinfo hr ON iw.HRId = hr.LoginId LEFT JOIN tblappplication ap ON iw.ApplicationId = ap.ApplicationId WHERE iw.STATUS='PE' and iw.CompanyId=$Cid";
                                                    $result = mysqli_query($conn, $sqln);
                                                    //      print_r(mysqli_fetch_assoc($result));
    ...
    <br><a href="#" class="btn btn-success   mr-1" data-toggle="modal" data-target="#myModala<?php echo $row['InterViewId']; ?>" style="margin-bottom: 5px"><i class="fa fa-check-circle-o"></i></a>
                                                    ...
                                                    <form method="post" action="InterView.php?IID=<?php echo $row['InterViewId']; ?>&Email=<?php echo  $row['Email']?>&rstatus=rstatus">
                                                    ...
                                                    <form method="post" action="InterView.php?IID=<?php echo $row['InterViewId']; ?>&Email=<?php echo $row['Email'];?>&sstatus=sstatus&JId=<?php echo $row['JobId']; ?>">
    ...
    if (mysqli_query($conn, $sqls)) {
        ...
        $e=new Email();
            ...
            $subject="JobPortal InterView Schedule";
                        $message="Your Interiew Response for now is try nect time ";
...
if (isset($_POST['iwapp'])) {
    ...
    $Em = $_GET['Email'];
        $e=new Email();
            ...
            $to=$Em;
            $subject="JobPortal InterView Schedule";
            $message="Your Interiew response is selected please visit same location ";
            $p= $e->send($from, $subject, $message, $to);
